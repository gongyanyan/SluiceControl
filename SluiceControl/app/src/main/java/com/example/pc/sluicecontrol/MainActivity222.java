...
public class MainActivity222 extends Activity implements View.OnClickListener{
    ...
    //Version ：     1、请求存起来(经过演示)----把请求的数据加入List集合
        ...
        public void handleMessage(Message msg){         // handle message
            ...
            switch (msg.what) {
                ...
                case 1:   //扇门
                    ...
                    Log.i("456",recLen+"状态循环::"+paramsBean2List.size());
                    if(recLen <= 0){     //isThough为true 人已经通过扇门，扇门关闭
                        ...
                        if(modeXMLBean.getType().equals("进站主机")||modeXMLBean.getType().equals("出站主机")){
                            ...
                            if(paramsBean2List.size()>0){   //说明还有请求没有发送完
                        ...
                        }else if(modeXMLBean.getType().equals("进站从机")||modeXMLBean.getType().equals("出站从机")){  //如果是从机，则需要告诉主机，线程已经结束
                                ...
                                postString("http://172.16.0.111:8082/",json);   //主机的地址
                                isOpenDoor = false;
                        }
            ...
            public void onOpenDoor(String param) {
                ...
                if(paramsBean.getMsgType().equals("FaceRecognitionResult")){   //执行开门命令
                    ...
                    if(modeXMLBean.getType().equals("进站主机")||modeXMLBean.getType().equals("出站主机")){
                        ...
                        if(isA.equals("初始值")&&!status.equals("80")){ //如果门开了，则不让再发开门指令
                            //记录已发请求的次数
                            openManiDoor();   //开主机门
                        }else {
                    ...
                    if(paramsBean.getMsgType().equals("出站从机")){   //开B门
                         ...
                         if(paramsBean.getOrderID().equals("从机循环结束")){
                            ...
                            if(paramsBean2List.size()>0){   //说明还有请求没有发送完
                             ...
                             if(isA.equals("初始值")&&!status.equals("80")){   //门没开就发送开门指令
                           ...
                           // Log.i("123",isOpened+"计时--"+paramsBean);
                            if (!isOpened) {
                             ...
                             /*   //门开后，发送请求，通知服务端(4.12IF9A-13 闸机进站通知)
                                NotiVerifyBean notiVerifyBean = new NotiVerifyBean(type(isA),"从机循环结束","chen",IpAndress,"","");
                                String json1 = JSONObject.toJSONString(notiVerifyBean);;
                                postString("http://120.52.9.5:18180/sttrade/ci/agm/notiVerifyResult",json1);   //主机的地址
                ...
                if(!status.equals("80")) {  //则发送请求给从机，结束跑秒进程，与切换图片
                    if (paramsBean.getMsgType() != null) {
                        if (paramsBean.getMsgType().equals("进站从机") || paramsBean.getMsgType().equals("出站从机")) {
                            ...
                            ParamsBean2 setBean = new ParamsBean2(isThough + "", "111", "chen",22, IpAndress,90,"0139");
                            String json = JSONObject.toJSONString(setBean);
    ...
    public void onClick(View view) {
        ...
        switch (view.getId()){
            case  R.id.image:
                ...
                //跳转维修测试界面
                Intent intent = new Intent(MainActivity222.this,RepairTetsActivty.class);
