...
import com.example.pc.sluicecontrol.common.NotiVerifyBean;
import com.example.pc.sluicecontrol.common.RequestAuthBean;
...
import com.example.pc.sluicecontrol.nanohttp.ParamsBean2;
...
public class MainActivity extends Activity implements View.OnClickListener{
    ...
    boolean isChuangru= false;  //判断是不是闯入
    String type="00";   //01进站、02出站

    List<ParamsBean2> paramsBean2List;  //记录请求开门的集合

    int recLen=0;
    TimerTask mTimerTask;
    Timer mTimer;
    ModeXMLBean modeXMLBean;
    ParamsBean2 paramsBean;   //得到http请求传入的参数
    int chaoshitime=15; //超时时间(默认时间)

    private long version = Long.valueOf("20181116");

    private final static String mainURL = "http://172.16.0.111:8082/";   //主机地址
    private final static String requestAuthURL = "http://120.52.9.5:18180/sttrade/ci/agm/requestAuth";   //IF9A-12 闸机请求进出站认证
    private final static String notiVerifyResultURL = "http://120.52.9.5:18180/sttrade/ci/agm/notiVerifyResult";   //4.12IF9A-13 闸机进站通知
        ...
        public void handleMessage(Message msg){         // handle message
            ...
            switch (msg.what) {
                ...
                case 1:   //扇门
                    ...
                    //Log.i("456",recLen+"状态循环::"+requestCount);
                    if(recLen <= 0){     //isThough为true 人已经通过扇门，扇门关闭

                        endMothod();
                        if(modeXMLBean.getType().equals("进站主机")||modeXMLBean.getType().equals("出站主机")){
                            if(paramsBean2List.size()>0){   //说明还有请求没有发送完

                                paramsBean =paramsBean2List.get(0);   //取出集合中第一个数据
                                ...
                                if(paramsBean.getMsgType().equals("出站从机")){   //开B门
                                    ...
                                    if (modeXMLBean.getType().equals("进站主机")){
                                        type ="01";
                                    }else if(modeXMLBean.getType().equals("出站主机")){
                                        type ="02";
                                    }
                                }
                                //openManiDoor();
                                RequestAuthMothd(type,paramsBean);
                                removeList(paramsBean);   //清除集合
                            }
                        }else {

                            //如果是从机，则需要告诉主机，线程已经结束
                            ParamsBean2 setBean = new ParamsBean2(modeXMLBean.getType(),"从机循环结束","chen",22,"88",90,"0139",IpAndress);
                            String json = JSONObject.toJSONString(setBean);;
                            postString(mainURL,json,"main");   //从机发送给主机
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        RequestAuthBean requestAuthBean = new RequestAuthBean("01","APP01201812141508295034","1");
        String json1 = JSONObject.toJSONString(requestAuthBean);
        postString(requestAuthURL,json1,"requestAuth");   //认证请求
        ...
        paramsBean = new ParamsBean2();
        paramsBean2List = new ArrayList<>();
...
//        startService(intent);     //启动后台服务
        ...
        mHttpServer.setOnOpenDoorListener(new HttpServerImpl.OnOpenDoorListener() {
            ...
            public void onOpenDoor(String param) {
                ...
                paramsBean = JSON.parseObject(param,ParamsBean2.class);   //请求的值
                ...
                if(paramsBean.getMsgType().equals("FaceRecognitionResult")){   //执行开门命令
                    ...
                    if(modeXMLBean.getType().equals("进站主机")||modeXMLBean.getType().equals("出站主机")){
                        ...
                        if(type.equals("00")&&!status.equals("80")){ //如果门开了，则不让再发开门指令
                            //记录已发请求的次数
                           // openManiDoor();   //开主机门
                            ...
                            RequestAuthMothd(type,paramsBean);    //认证请求

                        }else {
                            //把待请求的数据加入集合
                            paramsBean2List.add(paramsBean);
                        }

                    }else {

                        //发送POST请求给连门单元的PAD(主机)进行开门
                        ParamsBean2 setBean = new ParamsBean2(modeXMLBean.getType(),paramsBean.getOrderID(),paramsBean.getTenantID(),paramsBean.getUserType(),paramsBean.getUserID(),paramsBean.getScore(),paramsBean.getStationCode(),IpAndress);
                        String json = JSONObject.toJSONString(setBean);
                        postString(mainURL,json,"main");   //从机发送给主机
                    ...
                    SlaveIpAndress = paramsBean.getSalveIP();  //得到从机的IP地址
                    isThough = false;

                    if(paramsBean.getMsgType().equals("出站从机")){   //开B门

                         if(paramsBean.getOrderID().equals("从机循环结束")){
                            ...
                            if(paramsBean2List.size()>0){   //说明还有请求没有发送完
                             ...
                             if(type.equals("00")&&!status.equals("80")){   //门没开就发送开门指令
    ...
    public void removeList(ParamsBean2 paramsBean){
        //清除集合中已经调用Bean类
        Iterator<ParamsBean2> it = paramsBean2List.iterator();
        while(it.hasNext()){
            ParamsBean2 str = (ParamsBean2)it.next();
            if(str.getUserID().equals(paramsBean.getUserID())){  //说明是同一个人
                it.remove();
            }
        }
    }
    ...
    public void RequestAuthMothd(String type,ParamsBean2 paramsBean){
        Log.i("987",type+"--==--"+paramsBean);
        RequestAuthBean requestAuthBean = new RequestAuthBean(type,paramsBean.getOrderID(),paramsBean.getStationCode());
        String json1 = JSONObject.toJSONString(requestAuthBean);
        postString(requestAuthURL,json1,"requestAuth");   //认证请求

    }

    public void openOutSlave(){   //出站从机
        if(type.equals("00")||type.equals("02")){
            type="02";
            ...
            isOpened=false;
            isThough = false;
            ReadThrough(type);  //读取寄存器的值
            // setColor(CMD.ared);   //A红B绿
            readSatus();
        }
    }

    public void openInboundSlave(){   //进站从机
        if(type.equals("00")||type.equals("01")) {
            type = "01";
...
//                41 70 90 9B 80 80 80 88 80 80 8C 80 AD B7 E9      打开 80 88
                ...
                if(!Tools.isNull(receiveString)){   //不为空
                    ...
                    if (array.length>11&&(array[1].equals("78"))){
                        ...
                        String s = Util.hexStr2BinStr(array[4]).substring(4,8)+ Util.hexStr2BinStr(array[5]).substring(4,8);    //得到二进制数
                        chaoshitime = Integer.parseInt(s, 2);   //得到超时时间

                    }else if(array.length>11&&(array[3].equals("97")||array[3].equals("96"))){   //CNTB或CNTA
                        ...
                        if(status.equals("80")){  //说明门打开了
                           ...
                           // Log.i("123",isOpened+"计时--"+paramsBean);
                            if (!isOpened) {
                                ...
                                //门开后，发送请求，通知服务端(4.12IF9A-13 闸机进站通知)
                                Date ss = new Date();
                                //SimpleDateFormat sd = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
                                SimpleDateFormat sd = new SimpleDateFormat("yyyyMMdd HH:mm:ss");
                                String date = sd.format(ss.getTime());//得到当前时间

                                NotiVerifyBean notiVerifyBean = new NotiVerifyBean(type,paramsBean.getOrderID(),paramsBean.getTenantID(),date,"","");
                                String json1 = JSONObject.toJSONString(notiVerifyBean);;
                                postString(notiVerifyResultURL,json1,"notiVerifyResult");   //主机的地址
                                ...
                                if (paramsBean.getMsgType() != null) {
                                    ...
                                    if (paramsBean.getMsgType().equals("进站从机")) {
                                        ...
                                        setColor(CMD.agreen);   //A绿B红
                                        ParamsBean2 setBean = new ParamsBean2(status, "80", "chen",22,IpAndress,90,"0139");
                                        String json = JSONObject.toJSONString(setBean);
                                        postString("http://" + SlaveIpAndress + ":8082/", json,"salve");   //主机发送给从机
                    ...
                    }else if(array.length>6&&array[3].equals("94")){
                        //41 70 90 94 88 80 80 84 AD B4 ED
                        if (array[7].equals("84")){
                            isChuangru = true;
                        }
                    }
                ...
                if(!status.equals("80")) {  //则发送请求给从机，结束跑秒进程，与切换图片
                    if (paramsBean.getMsgType() != null) {
                        if (paramsBean.getMsgType().equals("进站从机") || paramsBean.getMsgType().equals("出站从机")) {
                            ...
                            ParamsBean2 setBean = new ParamsBean2(isThough + "", "111", "chen",22, IpAndress,70,"0139");
                            String json = JSONObject.toJSONString(setBean);
                            ...
                            if (!isThough) {  //没有通过
                                if(isChuangru){
                                    if(recLen>0){
                                        recLen = chaoshitime;    //非法闯入扇门关闭，超时时间从新开始
                                    }
                                }
                            }
                        }
                ...
                if(isThough){     //isThough为true 人已经通过扇门，扇门关闭

                   // Log.i("789",recLen+"状态循环::"+requestCount);
                    endMothod();
                    if(paramsBean2List.size()>0){   //说明还有请求没有发送完
...
//        serialPort.sendBuffer(messageTan.getBtAryTranData());
    ...
    public void endMothod(){    //结束方法(置为初始值)
        destroyTimer();
        stopThread();
        ...
        if(isFirst){  //第一次读到寄存器的值
            ...
            if(!sencendNum.equals(firstNum)){   //说明人已经过闸(人过闸计数器+1)
              ...
              /*  if(recLen>0){   //说明倒计时还没有结束， 结束了就不要再重新计时了，属于正常超时
                    recLen =22;   //重新计时(主机才生效)------人逆向想过闸，关门后要重新计时
                }*/
    ...
    public void onClick(View view) {
        ...
        switch (view.getId()){
            case  R.id.image:
                ...
                Intent intent = new Intent(MainActivity.this,RepairTetsActivty.class);
     ...
     * @param content 提交的内容
     */
    public void postString(String url, String content, final String requesttype) {
        //构建一个RequestBody对象,,因为提交的是json字符串需要添加一个MediaType为"application/json",
        // 普通的字符串直接是null就可以了

        //OkHttpClient okHttpClient = new OkHttpClient();
        Log.i("987",content+"-==="+"地址:"+url);
        ...
        okHttpClient.newCall(request).enqueue(new Callback() {
            ...
            public void onFailure(Call call, IOException e) {
                Log.i("987", "onFailure: " + e.getMessage());   //失败
            ...
            public void onResponse(Call call, Response response) throws IOException {
               ...
               if(requesttype.equals("requestAuth")){   //说明请求的是认证接口   4.11 IF9A-12 闸机请求进出站认证

                   ResponeBean responeBean = JSON.parseObject(string,ResponeBean.class);
                   //Log.i("987", "onResponse: " + string+"--==="+responeBean.getRetCode()+"==="+responeBean.getRetMsg());    //成功

                   if (responeBean.getRetCode().equals("0000")){  //认证成

                       if(paramsBean.getMsgType().equals("出站从机")){   //开B门
                           openOutSlave();
                       } if(paramsBean.getMsgType().equals("进站从机")) {   //开A门
                           openInboundSlave();
                       }else {
                           Log.i("987","主机开门");
                           openManiDoor();
                       ...
                       Looper.prepare();
                       Toast.makeText(MainActivity.this,responeBean.getRetMsg(),Toast.LENGTH_SHORT).show();
